...
- **JDBC Polling & Stuck Records:** When implementing the Transactional Outbox pattern via `JdbcPollingChannelAdapter`, we encountered a scenario where synchronization logs remained stuck in `PENDING`. This was traced to a `BadSqlGrammarException` caused by incorrect update SQL syntax when handling multiple records, combined with a missing `@EnableIntegration` annotation. We resolved this by using the `IN (:id)` syntax for status updates and explicitly enabling integration components, ensuring a reliable background sync flow.
